        ...
        <>
            <div className="flex justify-between items-center gap-3 px-4 py-2 ">
                <div className="flex w-full items-center truncate gap-4">
                    <Link to={`/search/${post.owner}`} className="w-14 h-14 aspect-square" aria-label={`vai alla pagina di ${post.owner}`}>
                        <img className="w-full h-full rounded-full object-cover" alt={`foto profilo di ${post.owner}`}  src={post.profilePicture} />
                    </Link>
                    <div className="flex flex-col gap-2">
                        <Link to={`/search/${post.owner}`} className="text-primary font-semibold text-lg w-fit h-fit" aria-label={`vai alla pagina di ${post.owner}`}>
                            {post.owner}
                        </Link>
